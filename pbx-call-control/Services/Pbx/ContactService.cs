...
public class ContactService : IContactService
{
    ...
    public ContaclListDataModel ContactList(int pageNumber, int pageSize)
    {
        ...
        using (IArrayDisposer<PhoneBookEntry> phoneBookEntry = PhoneSystem.Root.GetAll<PhoneBookEntry>().GetDisposer())
        {
            var contacts = phoneBookEntry.Select(x => new ContactDataModel(x.ID, x)).ToArray();

            var totalCount = contacts.Length;
            
            var items = contacts.Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
